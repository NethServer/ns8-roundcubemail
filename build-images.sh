...
roundcube_image="docker.io/roundcube/roundcubemail:1.7.3-apache"
...
buildah run --env="NODE_OPTIONS=--openssl-legacy-provider" nodebuilder-roundcubemail sh -c "cd /usr/src/ui && corepack enable && yarn install && yarn build"
...
buildah config --entrypoint=/ \
    ...
    --label="org.nethserver.images=docker.io/mariadb:10.11.19 ${roundcube_image}" \
    "${container}"
# Commit the image
